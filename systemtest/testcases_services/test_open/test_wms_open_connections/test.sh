...
# test WMS query
PETASCOPE_END_POINT="http://localhost:8080/rasdaman/ows?"
...
log "testing WMS open connections..."
for i in {1..100}
do
  ...
  numb_connection=$(psql -c "SELECT sum(numbackends) FROM pg_stat_database;" 2>&1 | awk 'NR==3{print $1}')
  log $i." Total connections: "$numb_connection

  if [[ $numb_connection == "" ]]; then
  	log "maximum connections, error is not fixed."
    NUM_SUC=$(($NUM_SUC + 1))
  fi
done
