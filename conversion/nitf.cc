...
#include "nitf/nitf.h"
#include "nitf/image.h"

r_Conv_NITF::r_Conv_NITF( const char *src, const r_Minterval &interv, const r_Type *tp )
throw(r_Error):r_Convert_Memory(src, interv, tp)
{
}

r_Conv_NITF::r_Conv_NITF( const char *src, const r_Minterval &interv, int tp )
throw(r_Error):r_Convert_Memory(src, interv, tp)
{
}

r_Conv_NITF::~r_Conv_NITF( void )
{
}

/***********************************************************
 * NAME: r_Conv_NITF::convertFrom
 * ---------------------------------------------------------
 * PURPOSE: convert from NITF image file data to the
 * corresponding rasdaman type pixel sequential data
 ***********************************************************/

r_convDesc& r_Conv_NITF::convertFrom( const char *options ) throw(r_Error)
{
    ...
    {
        LFATAL << "r_Conv_NITF::convertTo(): out of memory!";
    ...
    {
        LFATAL << "r_Conv_NITF::convertFrom:" << "unsupported NITF file pixel type";
...
/***********************************************************
 * NAME: r_Conv_NITF::convertTo
 * ---------------------------------------------------------
 * PURPOSE: convert from rasdaman data to the
 * corresponding nitf storage type
 ***********************************************************/

r_convDesc& r_Conv_NITF::convertTo( const char *options ) throw(r_Error)
{
...
r_Convertor* r_Conv_NITF::clone( void ) const
{
    return new r_Conv_NITF(desc.src, desc.srcInterv, desc.baseType);
}

const char * r_Conv_NITF::get_name( void ) const
{
    return format_name_ntf;
}

r_Data_Format r_Conv_NITF::get_data_format( void ) const
{
    return r_NITF;
}
