...
string StackTrace::StackTraceEntry::toString(long offset) const
{
    stringstream ss;
    ss << std::setw(5) << ("[" + std::to_string(static_cast<long>(m_index) - offset) + "] ")
       << getFileLoc()
...
string StackTrace::toString(long offset) const
{
    stringstream ss;
                              ...
                              string &address) const
{
    ...
    if (!mangled_name || !offset_begin || mangled_name >= offset_begin || !address_begin)
    {
        return;
    }
