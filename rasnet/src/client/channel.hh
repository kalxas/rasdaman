...
{
/**
 * @brief The Channel class Implementation of google::protobuf::RpcChannel
...
public:
     ...
     * @param serverAddress The server endpoint to which this channel will connect
...
private:
    ...
    boost::uint64_t callCounter;/*!< Counter used to assign unique ID's to each method call */
    boost::mutex callCounterMutex;/*!< Mutex used to secure synchronize access to the counter */
    ...
    /**
     * @brief cleanup If an exception is thrown in the constructor,
     * for example when failing to connect to the server, the destructor is not called,
     * but cleanup must still be performed.
     */
    void cleanup();
    ...
    /**
     * @brief workerMethod This method runs in a separate thread
     * attached to the Channel that maintains communication with the server
     */
    void workerMethod();
};
}
