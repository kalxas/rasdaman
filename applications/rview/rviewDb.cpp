...
int rviewDatabase::lookupCollection(collection_desc *desc)
{
    ...
    catch ( r_Error &errObj )
    {
        ...
        else
        {
            switch (errObj.get_kind())
            {
            case r_Error::r_Error_ClientUnknown:
                msg = lman->lookup("errorClientUnknown");
                break;
            case r_Error::r_Error_DatabaseClosed:
                msg = lman->lookup("errorDatabaseClosed");
                break;
            case r_Error::r_Error_QueryParameterCountInvalid:
                msg = lman->lookup("errorQueryParamNum");
                break;
            case r_Error::r_Error_TransferFailed:
                msg = lman->lookup("errorTransferFailed");
                break;
            default:
                msg = lman->lookup("errorUnknown");
                break;
            }
        }
...
r_Fast_Base_Scale *rviewDatabase::lookupScaledObject(collection_desc *desc, double scale)
{
    ...
    desc->number = 1;
    desc->mddObjs[0].mdd = mddObj;
    desc->mddObjs[0].flags = 0;
            ...
            else
            {
                ...
                for (i=0; i<collMembers; i++, iterator++)
                {
                    ...
                    case r_Type::POINTTYPE:
                        ((r_Ref<r_Point>)*iterator)->print_status(memstr);
                        break;
                    case r_Type::SINTERVALTYPE:
                        ((r_Ref<r_Sinterval>)*iterator)->print_status(memstr);
                        break;
                    case r_Type::MINTERVALTYPE:
                        ((r_Ref<r_Minterval>)*iterator)->print_status(memstr);
                        break;
                    case r_Type::OIDTYPE:
                        ((r_Ref<r_OId>)*iterator)->print_status(memstr);
                        break;
                    default:
                        ((r_Ref<r_Scalar>)*iterator)->print_status(memstr);
                        break;
                    }
                    memstr << '\0';
    ...
    catch ( r_Equery_execution_failed &errObj )
    {
        ...
        else
        {
            ...
            if (what != NULL) // might actually happen...
            {
                ...
                line = errObj.get_lineno();
                col = errObj.get_columnno();
