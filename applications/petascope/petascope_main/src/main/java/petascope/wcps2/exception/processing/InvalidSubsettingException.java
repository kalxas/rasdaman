...
public class InvalidSubsettingException extends WCPSException {
    ...
    public InvalidSubsettingException(String axisName, ParsedSubset<String> subset) {
        super(subset.isTrimming() ? ERROR_TEMPLATE.replace("$subsetDomainType", "subsetting").replace("$subsetBound", subset.getLowerLimit() + ":" + subset.getUpperLimit()).replace("$axis", axisName)
              : ERROR_TEMPLATE.replace("$subsetDomainType", "slicing").replace("$subsetBound", subset.getSlicingCoordinate()).replace("$axis", axisName), ExceptionCode.InvalidSubsetting);
    }
