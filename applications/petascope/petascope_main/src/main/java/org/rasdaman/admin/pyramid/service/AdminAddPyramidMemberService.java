 ...
 * nested pyramid members of it to a base coverage)
 *
 * API: /rasdaman/admin/coverage/pyramid/add
        COVERAGEID={base-coverage}
        MEMBERS={pyramid-element-coverage-list}
 *
 * Example: /rasdaman/admin/coverage/pyramid/add?COVERAGEID=Sentinel2_10m &
 * MEMBERS=Sentinel2_20m,Sentinel2_60m
 *
 *
 *
 ...
 */
@Service
public class AdminAddPyramidMemberService extends AbstractAdminService {

    private static final org.slf4j.Logger log = LoggerFactory.getLogger(AdminAddPyramidMemberService.class);

    private static Set<String> VALID_PARAMETERS = SetUtil.createLowercaseHashSet(KEY_COVERAGE_ID, KEY_MEMBERS, KEY_PYRAMID_HARVESTING);
    ...
    private String[] parsePyramidMemberCoverageIds(Map<String, String[]> kvpParameters, GeneralGridCoverage baseCoverage) throws PetascopeException {
        ...
        String[] pyramidMemberCoverageIds = getValueByKey(kvpParameters, KEY_MEMBERS).split(",");
        ...
        for (String pyramidMemberCoverageId : pyramidMemberCoverageIds) {
            if (baseCoverage.hasPyramidMember(pyramidMemberCoverageId)) {
                throw new PetascopeException(ExceptionCode.InvalidRequest,
                        "Coverage '" + pyramidMemberCoverageId + "' already exist in the pyramid of base coverage '" + baseCoverage.getCoverageId() + "'.");
            }
        }
            
        return pyramidMemberCoverageIds;
    }

    /**
     * Check if pyramid member coverage has the same structure types for
     * baseCoverage
     */
    ...
    /**
     * Add the pyramidMemberCoverage (covA) to the base coverage (covB) as as
     * suitable scale levels. NOTE: - pyramidMemberCoverage can be federated
     * coverage - any pyramid members of covA (recursively for any pyramid
     * member of pyramid members of cov A and so on...) will be added to covB
     */
    ...
    /**
     * Find all coverages containing a pyramid member coverage id, then add this
     * pyramid member coverage to all these containing coverages.
     */
