...
public class GdalParameters {
    ...
    // a coverage should not add georeference if it is set to false
    private boolean georeferenced = true;

    /* Members */
    // Creation options (extraParams)
    /**
     * GDAL creation option for the codec. In JP2OpenJPEG there are two
     * alternatives: JP2/J2K, the latter not supporting GMLJP2.
     */
    ...
    public GdalParameters(CoverageExpr queryRoot, Integer expectedDim) throws WCPSException {
        ...
        if (info != null) {
                ...
                if (!queryRoot.isSlicedAxis(dimName)) {
                    ...
                    for (IRasNode subset : subsets) {
                        try {
                            ...
                            log.debug("Cannot cast Min, Max value of axis name: " + dimName + " to double.");
                            nameToBounds.remove(dimName);
                        }
                    }
                }
            ...
            // This is bounding box xmin;xmax;ymin;ymax for georeference (only set when in 3D case: Long, Lat, Time or 2D case Long, Lat).
            // Set the bounds in the proper order (according to the order of the axes in the coverage
            if (nameToBounds.size() == expectedDim) {
                String firstAxis = nameToBounds.keySet().toArray()[0].toString();
                String lastAxis = nameToBounds.keySet().toArray()[1].toString();

                Double[] dom = nameToBounds.get(firstAxis);
                lowX = dom[0];
                highX = dom[1];

                dom = nameToBounds.get(lastAxis);
                lowY = dom[0];
                highY = dom[1];
    ...
    /**
     * Add configuration KV-pair for GDAL. Unset a configuration option by
     * specifying null or empty value. All values in the map are ensured not to
     * be null or empty.
     *
     * @param key
     * @param value
     ...
     * @return The list of configuration options: "config=key1 value1, key2
     * value2, __, keyN valueN".
     */
