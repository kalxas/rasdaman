 ...
 */
public class LegacyDomainElement implements Cloneable {

    private static Logger log = LoggerFactory.getLogger(LegacyDomainElement.class);
    ...
    throws Exception {
        ...
        if (axisLabel == null) {
            throw new Exception("Invalid domain element: Element name and type cannot be null.");
        }
        ...
        if ((min != null) && (max != null)) {
            ...
            throw new Exception("Invalid domain element: Element name cannot be empty.");
        }
        ...
        if (!isIrregular) {
            ...
            if (maxValue.compareTo(minValue) < 0) {
                throw new Exception("Invalid domain element: upper-bound is greater then lower-bound.");
            }

            BigDecimal diffBD = maxValue.subtract(minValue).add(CrsUtil.isGridCrs(nativeCrs) ? BigDecimal.ONE : BigDecimal.ZERO);
            scalarResolution  = LegacyBigDecimalUtil.divide(diffBD, new BigDecimal(dimensionality));
        }
    ...
    @Override
    public LegacyDomainElement clone() {
        ...
        try {
            ...
            LegacyDomainElement cloned = new LegacyDomainElement(
                cloneMin,
                cloneMax,
                cloneLabel,
                cloneType,
    ...
    public boolean equals(LegacyDomainElement de) {
    ...
    public BigDecimal getMaxValue() {        
        return petascope.util.BigDecimalUtil.stripDecimalZeros(maxValue);
    }

    public BigDecimal getMinValue() {
        return petascope.util.BigDecimalUtil.stripDecimalZeros(minValue);
    }
    ...
    public boolean timeAxis() {
        return type.equals(LegacyAxisTypes.T_AXIS);
    }
}
