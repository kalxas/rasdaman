...
import org.rasdaman.migration.legacy.LegacyBigDecimalUtil;
 ...
 */
public class LegacyAllowedValue {
     ...
     */
    private final List<LegacyRealPair> intervals;
    /**
     * SIGNIFICANT FIGURES.
    ...
    // constructor
    public LegacyAllowedValue(List<LegacyRealPair> intervalList) {
        significantFigures = null;
        intervals = new ArrayList<LegacyRealPair>(intervalList.size());
        for (LegacyRealPair interval : intervalList) {
     ...
     */
    public Iterator<LegacyRealPair> getIntervalIterator() {
        return intervals.iterator();
    }
    // public Integer getSignificantFigures() { ... }
    
    public List<LegacyRealPair> getIntervals() {
    ...
    public Element toGML() {
        ...
        Element interval;
        Iterator<LegacyRealPair> intervalIt = getIntervalIterator();
        LegacyRealPair pair;
        ...
        if (intervalIt.hasNext()) {
            while (intervalIt.hasNext()) {
                ...
                interval.appendChild(LegacyBigDecimalUtil.stripDecimalZeros(pair.getMin()) + " " + LegacyBigDecimalUtil.stripDecimalZeros(pair.getMax()));
