  ...
  private static Logger log = LoggerFactory.getLogger(Resolver.class);
    ...
    public String parseGML(String gmlDefinition) throws SecoreException
    {
        ...
        try {
            ...
            error = e.getMessage();
            throw new SecoreException(ExceptionCode.ParserConfigurationException, " Error in parser configuration: " + e.getMessage());
        } catch (SAXException e) {
            error = e.getMessage();
            throw new SecoreException(ExceptionCode.SAXParserException, " XML is not well-formed: " +  e.getMessage());
        } catch (IOException e) {
            error = e.getMessage();
            throw new SecoreException(ExceptionCode.IOConnectionError, " The request could not be retrieved: " + e.getMessage());
        }
        return error;
    }
