...
ServerConfig::ServerConfig(const std::string &hostName, const std::int32_t &port, std::shared_ptr<DatabaseHost> dbHost):
...
std::int32_t ServerConfig::getPort() const
{
    return port;
}

void ServerConfig::setPort(const std::int32_t &value)
{
    port = value;
}
